...
pub async fn run(config: BridgeConfig) -> anyhow::Result<()> {
    ...
    // Escuchar en IPv4 e IPv6 loopback para máxima compatibilidad (Windows
    // WebView2 puede preferir ::1 sobre 127.0.0.1).
    ...
    // IPv6 puede fallar si el adaptador loopback IPv6 no está habilitado —
    // no es crítico, continuamos solo con IPv4.
...
fn enforce_business(state: &AppState, headers: &HeaderMap) -> Result<(), AppError> {
    let origin = headers
        ...
        .unwrap_or("");
    let origin_allowed =
        !origin.is_empty() && state.config.allowed_origins.iter().any(|o| o == origin);
    if !origin_allowed {
        return Err(AppError {
    ...
    if provided == paired_biz {
        ...
        Err(AppError {
            status: StatusCode::FORBIDDEN,
            message: format!("Este equipo está vinculado a otro negocio (esperado: {paired_biz})"),
        })
    }
}
